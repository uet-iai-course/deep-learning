...
2. **Lượt tự kiểm:** chỉ đọc các tệp vừa ghi và checklist ngắn; không đọc lại toàn bộ dossier. Tách lượt này thành hai task nếu cần cả sửa và báo cáo: task sửa chỉ ghi lại **một** tệp đích; task báo cáo chỉ tạo `writer-report.md` và không sửa sản phẩm.
3. Mỗi task phải liệt kê chính xác tệp được đọc, tệp được ghi và thứ tự ưu tiên. Một task không được vừa thực hiện nhiều lệnh thay thế cục bộ vừa tạo tệp thứ hai. Khi có nhiều sửa nhỏ trong một tệp, writer phải gom thành một lần ghi lại toàn tệp; khi chỉ có tối đa ba thay thế đã khóa, mới dùng thay thế cục bộ. Yêu cầu writer ghi checkpoint/tệp đích ngay sau khi đã có nội dung hợp lệ, không dành một vòng dài để suy luận toàn bộ rồi mới ghi.
...
10. Với bản nháp lecture note, mẫu ổn định từ Buổi 02 là **hai đầu vào, một đầu ra**: lượt soạn chỉ nhận `approved-spec.md` và mẫu lecture note, chỉ ghi `lecture-note.md`; lượt tự kiểm dùng staging mới, chỉ nhận bản nháp và checklist, rồi chỉ ghi lại một bản đã sửa. Không gộp báo cáo, planning, index hoặc SVG vào hai lượt này. Nếu cần tạo SVG mới, chạy một task riêng cho từng nhóm tài sản đã khóa sau khi nội dung và nhãn hình được duyệt.
