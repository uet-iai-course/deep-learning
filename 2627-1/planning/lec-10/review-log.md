...
- Kiểm tra tĩnh của ghi chú xác nhận một H1, 28 dòng chỉ thị tạo 7 cặp câu hỏi–lời giải, 135 biểu thức KaTeX dựng được với `throwOnError: true` và `strict: "error"`, bảy SVG tồn tại và đủ `role="img"`, `title`, `desc`. Không có mã trang, metadata tác tử, ký tự thay thế hoặc ký tự Cyrillic trong nội dung công khai.
- Chromium headless duyệt viewer ở $1280\times720$ và $390\times844$: 135 biểu thức, bảy hình, bảy khối lời giải đóng mặc định, không lỗi console, lỗi trang, yêu cầu hỏng hay ảnh hỏng. Liên kết deck, bỏ qua điều hướng, mở lời giải bằng bàn phím, chế độ in, chặn vượt thư mục và chặn ghép nhầm bài đều đạt. Chênh 35 px của `body.scrollWidth` trên màn hình hẹp nằm trong khối cuộn công thức; cửa sổ không bị cuộn ngang.
