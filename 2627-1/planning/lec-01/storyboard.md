...
| Giới hạn affine | L01-10 | L01-11–12 | L01-13 | L01-14–16 | Không áp dụng: bài chưa chuyển sang forward pass của MLP | L01-13 |
| MLP và kích hoạt | L01-15 | L01-16–17 | L01-18, L01-20–22 | L01-18, L01-20–22 | L01-18 và L01-22 nối công thức với forward pass, tầng ra | L01-18, L01-20, L01-23 |
| ReLU và XOR | L01-24–25 | L01-26–28 | L01-29–30 | L01-29–31 | Forward pass toàn batch ở L01-30 | L01-30–31, L01-33 |
...
Các cụm học từ dữ liệu, MLP–kích hoạt và ReLU–XOR dùng chu trình đầy đủ hoặc gần đầy đủ. Cụm giới hạn affine dùng chu trình rút gọn vì chưa có triển khai MLP. Cụm mở rộng cũng rút gọn: L01-X05 minh họa biểu diễn phân tán, không được gán là bước triển khai.

Kiến thức đầu vào là phép nhân ma trận và phân loại nhị phân. Sản phẩm học tập cuối là lời giải thích kiểm chứng được về việc MLP giải XOR, kèm shape và số tham số. Bốn điểm XOR giữ nguyên từ L01-13 đến L01-31. Mỗi hàng của $X$ là $x_i^\top$; các ký hiệu $X,W_1,b_1,H,W_2,b_2,Z$ giữ nguyên từ công thức tổng quát sang ví dụ số.
...
| L01-10 | 4 | Biên affine là siêu phẳng | $\mathbf x,\mathbf w,b$ → $z=0$ | Một đơn vị tính biên này như thế nào |
...
| L01-14 | 3 | Nhiều đơn vị tạo một tầng ma trận | Hàng $x_i^\top$, $X,W,b$ → $Y$ và shape | Thử xếp hai tầng |
| L01-15 | 5 | Hai tầng affine rút thành một tầng affine | $X,W_1,b_1,W_2,b_2$ → phép affine tương đương | Thêm phép không affine |
| L01-16 | 3 | Kích hoạt phi tuyến chặn phép rút gọn | Tiền kích hoạt → biểu diễn phi tuyến | Đặt cơ chế vào cấu trúc MLP |
| L01-17 | 4 | MLP có đầu vào, tầng ẩn và tầng ra | Các đơn vị → kiến trúc 2 tầng | Theo dõi tensor qua mạng |
| L01-18 | 5 | Forward pass phải bảo toàn shape hợp lệ | $B,d,h,k$ → shape từng tensor | Nghỉ ngắn rồi chọn hàm kích hoạt |
| L01-19 | 2 | Tổng quan vai trò và miền ra của activation | Tiền kích hoạt → ba họ hàm | Xét ReLU |
...
| L01-22 | 3 | Hàm tầng ra phụ thuộc miền đích | $Z:B\times k$ → sigmoid hoặc softmax theo lớp | Kiểm tra lựa chọn |
| L01-23 | 2 | Chọn activation theo vị trí và bài toán | Ba tình huống → ReLU, sigmoid, softmax | Quay lại tác động hình học của ReLU |
| L01-24 | 3 | Đặt bài toán biến đổi XOR sang không gian ẩn | Bốn điểm ở X → chuỗi affine rồi ReLU | Xét riêng phần affine |
| L01-25 | 3 | Affine tạo đúng $A=(0,-1),(1,0)\times2,(2,1)$ nhưng chưa tách lớp | Điểm lớp 1 nằm trên bao lồi lớp 0 | Thêm ReLU |
| L01-26 | 5 | ReLU ép miền âm lên biên tọa độ | $a_j$ → $h_j$ và các điểm H, chưa có biên | Quan sát khả năng tách |
| L01-27 | 3 | Tầng ra dùng biên affine trong không gian ẩn | $H$ → biên xuất hiện | Kéo biên về không gian gốc |
...
| L01-30 | 6 | Tính một hàng trước rồi hiện cả batch | $X:4\times2$ → $H$, logit, $p$, $\hat y$ | Kiểm tra shape và độ lớn mô hình |
| L01-31 | 3 | Batch không làm thay đổi số tham số | Shape tensor → 9 tham số | Mở sang mạng sâu hơn |
| L01-32 | 2 | Mạng sâu hơn hợp thành nhiều biểu diễn | $H_{\ell-1}$ → $H_\ell$ | Chốt tuyến lõi |
| L01-33 | 3 | Kiểm cả lý do XOR cần phi tuyến và shape | XOR, $32\times10$, $h=20$, $k=3$ → lời giải thích và phép tính | Kết thúc tuyến 100 phút |
...
|---|---:|---|---|---|
| L01-X01 | 3 | Kiểm shape và cách đếm của mạng ba tầng tham số | `lec05_multilayer.pdf` tr. 35 | Phân biệt hai trục kiến trúc |
| L01-X02 | 4 | Độ sâu và độ rộng thay đổi cấu trúc theo hai cách | `lec05_multilayer.pdf` tr. 29, 35 | Xem minh họa biên nhiều mảnh |
| L01-X03 | 3 | Biên nhiều mảnh chỉ là minh họa định tính | `lec05_multilayer.pdf` tr. 29 | Đặt giới hạn cho kết luận biểu diễn |
| L01-X04 | 3 | Nguồn chỉ nêu mệnh đề xấp xỉ phổ dụng khái quát | `lec05_multilayer.pdf` tr. 29 | Sức biểu diễn không đồng nghĩa dễ học |
...
| Vẽ MLP và ghi shape | 20 | Sơ đồ batch-first cùng shape mọi tham số và tensor |
| So sánh affine chồng tầng với MLP ReLU | 15 | Phép rút gọn đại số và một phản ví dụ phi tuyến |
| Quiz | 5 | Bốn câu ngắn về thuật ngữ, activation và số tham số |
