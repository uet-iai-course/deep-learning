...
- giải thích giới hạn hình học của bộ phân loại affine qua XOR;
- mô tả đầu vào, tầng ẩn, tầng ra và hàm kích hoạt của mạng perceptron đa lớp (MLP);
- thực hiện forward pass batch-first, kiểm tra shape và tính số tham số;
...
4. Mô hình affine tạo biên phẳng; XOR cho thấy giới hạn.
5. Xếp chồng các phép affine không thay đổi giới hạn đó.
6. Hàm không affine đặt giữa các tầng nói chung chặn phép rút gọn.
7. MLP hiện đại dùng đơn vị ẩn có activation, tạo logit rồi đổi thành đầu ra phù hợp.
8. ReLU biến đổi hình học dữ liệu; tầng ra tách trong không gian ẩn.
9. Mạng ReLU 2–2–1 cho một forward pass kiểm chứng được trên XOR.
10. Tuyến mở rộng kiểm shape qua ba tầng tham số rồi nối độ sâu, độ rộng, biên nhiều mảnh, xấp xỉ phổ dụng và biểu diễn phân tán.

Mạch này đã được rà theo Quill: **vấn đề → khung học → giới hạn affine → XOR → phi tuyến → MLP → sức biểu diễn và giới hạn**. Cầu nối vector cột của slide nguồn sang batch-first được khóa bằng quy ước mỗi hàng $X$ là $x_i^\top$. Không khởi tạo `quill.json`.
...
| Mô hình affine và XOR | L01-10–16 | `lec02_linear_part1.pdf`, tr. 15–21; `lec05_multilayer.pdf`, tr. 4–10 | Giáo trình PDF tr. 83–86 |
...
| Mở rộng: sức biểu diễn và giới hạn | L01-X01–X06 | `lec05_multilayer.pdf`, tr. 29–35 | X01 kiểm shape ba tầng; không lặp hình hoặc luận điểm cầu nối ở L01-32 |
...
2. Vẽ MLP và ghi shape tensor, 20 phút.
3. So sánh chuỗi tầng affine với MLP dùng ReLU, 15 phút.
