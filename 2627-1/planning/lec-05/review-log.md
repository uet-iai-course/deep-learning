...
- Dossier chỉ chứa phần DOCX Buổi 5 và các dải đã duyệt: `lec09_cnn_architectures.pdf` PDF 3–9, 11–23, 26–34, 44–46; `lec10_training.pdf` PDF 25–32; GT PDF 132–146 và 149–163. Tệp nhị phân được trích cục bộ thành UTF-8; không có `.env`, bí mật hoặc symlink trong dossier.
- Vai lập kế hoạch, bản đồ nguồn A và semantic contract chạy bằng `z-ai/glm-5.3-flash`; runtime xác nhận model yêu cầu và thực tế trùng nhau, provider OpenRouter. Vai giáo trình–toán đầu tiên hết 300 giây trước khi trả báo cáo nên bị loại toàn bộ; lượt thay thế dùng cùng model/provider và phạm vi hẹp hơn đã hoàn tất.
- Hai bản đồ chủ đề được hợp nhất thành L05-T01–T10 trong `outline.md`. Giữ tuyến LeNet → AlexNet → VGG → GoogLeNet/Inception → BN → ResNet → học chuyển giao; gộp chi tiết mở rộng thành L05-T09; T10 chỉ ghi ranh giới đọc, không triển khai nội dung ngoài phạm vi.
- Bác các suy đoán do vai giáo trình thiếu nguồn slide: 49K² so với 27K² là phép so một nhân 7×7 với ba nhân 3×3 theo slide đã duyệt, không phải mệnh đề 5×5 của GT; 61.100.840 tham số, 716.767.232 MAC, 163.328 trọng số Inception, 6272 giá trị/kênh BN và 589.824/69.632 đều đã được tái lập từ đặc tả đầy đủ trong slide/deck và giữ nguyên.
- Kế hoạch worker được điều phối viên siết lại: phân tích các mạch có thể song song, nhưng mọi lượt DeepSeek ghi mảnh lecture note chạy tuần tự, staging riêng, `MCP_WRITE_POLICY=create-once`, mặc định không quá 2.500 ký tự. Không cho một writer soạn nhiều mảnh hoặc tự kiểm toàn văn trong cùng task.
- Bốn lượt DeepSeek đầu tiên trả lần lượt 3.868, 3.752, 2.581 và 2.815 ký tự, vượt ngân sách 2.500 ký tự. Cả bốn đầu ra bị loại toàn bộ, không hợp nhất và không dùng làm đầu vào cho lượt sau.
- Giới hạn mềm được thay bằng giới hạn cứng `MCP_MAX_WRITE_CHARS=2500` trong bridge: lượt ghi quá trần bị từ chối trước khi tạo tệp. Bộ kiểm thử bridge có 16 ca và đều đạt; README cùng prompt gốc đã ghi cơ chế này để các đợt sau không mở rộng phạm vi ngoài ý muốn.
- Mười hai mảnh thay thế chạy tuần tự trong mười hai staging root mới, dùng `deepseek/deepseek-v4-flash-0731`, provider OpenRouter, `create-once` và giới hạn cứng. Mỗi mảnh dài 1.208–2.493 ký tự; không có ký tự thay thế Unicode hoặc ký tự Cyrillic lẫn vào tiếng Việt. Codex chỉ lấy các dữ kiện phù hợp đặc tả đã duyệt và biên tập lại trước khi ghép.
- Lượt DeepSeek tự kiểm cuối chỉ đọc lecture note và checklist, ghi một báo cáo 1.975 ký tự trong staging mới; model/provider được xác nhận đúng và báo cáo đạt các con số lõi. Báo cáo không tự sửa tệp dự án.
- Năm lượt phản biện lecture note được chia theo góc nhìn sinh viên, chuyên gia Học sâu, độ chính xác, phản biện học thuật và mạch viết. Bốn lượt đầu hoàn tất đúng `z-ai/glm-5.3-flash`/OpenRouter; lượt mạch đầu vượt tám tool-call nên bị loại toàn bộ và chạy lại trên staging chỉ có note/storyboard.
- Giữ các nhận xét có bằng chứng: nêu tiên quyết Bài 02–03, vùng đệm 2 ở tích chập đầu AlexNet, quy ước MAC không tính cộng độ lệch, bảng tái lập tổng MAC, phạm vi $C_{in}=192$ của số 163.328, phép so đầu GoogLeNet với FC6 VGG, câu nối AlexNet→VGG và ResNet→học chuyển giao, ví dụ BN trước công thức. Bác lỗi asset do staging phản biện cố ý không chứa SVG và bác tổng MAC 716.807.232 vì phép tính đó dùng sai tích chập 3; tổng đúng vẫn là 716.767.232.
