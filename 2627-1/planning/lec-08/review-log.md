...
- Kiểm định tĩnh lecture note: một `h1`, 40 chỉ thị bài tập/đáp án, 178 biểu thức KaTeX, chín ảnh và không có lỗi cấu trúc. Chromium dựng thành công ở $1280\times720$ và $390\times844$: 178 công thức, chín SVG, mười khối đáp án đóng mặc định, không lỗi runtime, không tràn ngang; bàn phím, chế độ in, chặn đường dẫn vượt thư mục và chặn ghép sai buổi đều đạt.
...
## Pipeline cập nhật slide deck theo lecture note

- Ba vai đọc độc lập dùng đúng `z-ai/glm-5.3-flash` qua OpenRouter để rà khớp nguồn/note, độ chính xác toán–shape–triển khai và văn phong công khai/ghi chú. Các kết quả được chấp nhận có `requested_model` và `observed_model` trùng khớp, `provider` là `OpenRouter`.
- DeepSeek dùng đúng `deepseek/deepseek-v4-flash-0731` qua OpenRouter và chỉ tạo `suggestions.md` trong staging riêng. Phạm vi tiếp tục bị khóa bởi `MCP_WRITE_POLICY=create-once` và `MCP_MAX_WRITE_CHARS=2500`; lần ghi đầu 3.297 byte bị từ chối toàn bộ, lượt 2.082 byte mới được chấp nhận. DeepSeek không sửa HTML, lecture note hoặc tệp dự án.
- Thay đổi cục bộ: tầng chiếu từ vựng dùng $W_y,b_y$ để không xung đột với cổng ra LSTM; giữ công thức softmax đúng $A_{t'}$; làm sạch các cụm mô tả quy trình soạn; bổ sung tiên quyết nhân ma trận theo lô; giải thích ký hiệu chữ thường của ví dụ vô hướng; nêu giả thiết cùng chiều ẩn để $Q$ khởi tạo $S_0$; đổi chỉ số dừng sinh sang $t'$; bỏ điều kiện bộ nhớ không được dùng trong đáp án.
- Năm góc phản biện độc lập đã được thực hiện. Lượt học thuật đầu tiên vượt giới hạn tám vòng công cụ nên bị loại và được thay bằng lượt rà hẹp thành công. Vai mạch viết xác nhận bảy mạch ngoài, 44 trang và các kết nối vào/ra. Nhiều báo cáo đọc sai $A_{t'}$ thành $A_t$; kiểm tra trực tiếp HTML và KaTeX xác nhận bản hiện hành luôn dùng đúng $A_{t'}$, nên không áp dụng đề xuất sai. Tuyên bố tổng lõi 98 phút cũng bị bác: cộng trực tiếp bảng storyboard cho đúng 100 phút; phần mở rộng 20 phút.
- Không thêm lab TensorFlow/PyTorch vì nguồn đã chọn không có code tương ứng; quy tắc dự án cấm tự tạo demo ngoài nguồn. Hoạt động thực hành của buổi vẫn nằm trong bài tập 50 phút đã tách riêng.
- `$no-ai-slop` được áp dụng cho toàn bộ mặt slide và 44 khối ghi chú: không còn tên model/worker/pipeline, nhãn nội bộ, chỉ dẫn cho diễn giả/người viết, câu quảng bá hoặc mô tả tự kiểm. Nguyên tắc `$quill` xác nhận tuyến gradient dài → cơ chế LSTM → ví dụ → gradient → GRU → kiến trúc → dịch máy → đối chiếu → kết luận; không tạo `quill.json`.
- Kiểm định cuối dựng 44 trang ở cả $1280\times720$ và $960\times720$, tạo 88 ảnh: bảy mạch ngoài, 44 mã duy nhất, 44 ghi chú, 208 biểu thức KaTeX và chín SVG; không lỗi phân tích, tài nguyên, tràn hay runtime. Sáu ranh giới phím Phải và đích phím End đều đúng. Lecture note được dựng lại sau đồng bộ ở khung rộng/hẹp và chế độ in, không phát sinh lỗi.
- `python3 -m reloadserver 8765` vẫn không khả dụng; kiểm định dùng máy chủ tĩnh cục bộ trên cổng 8766. Codex Slides không có runtime trong môi trường, vì vậy bước rà trực quan dùng Chromium headless và không tuyên bố đã dùng plugin.
