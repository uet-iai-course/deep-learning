...
|---|---|---|---|---|
| 1 | BC và dữ liệu trình diễn (L13-00–07, 8 trang, 24 phút) | Đặt vấn đề học từ chuyên gia; dựng $\mathcal D_E$, loss BC, hợp đồng huấn luyện/đánh giá | Từ bài trước: hồi quy và entropy chéo | Giao $\mathcal D_E$, loss BC và hợp đồng đánh giá quỹ đạo cho mạch 2 |
...
| 3 | Từ BC sang RL: tương tác và MDP (L13-13–20, 8 trang, 19 phút) | Quyết định BC/RL; vòng tương tác; năm thành phần LLO26 và chính sách; tuple MDP; Markov; lợi tức; cờ $D^{term}$ | Nhận câu hỏi thiếu nhãn từ mạch 2 | Chuyển giao lợi tức $G_t$ và $D^{term}$ cho mạch 4 |
...
| 7 | Tổng kết và phụ lục (L13-41; X01–X04, 5 trang, 2 phút lõi + 20 phút phụ) | Phân biệt hai nguồn tín hiệu (hành động chuyên gia; phần thưởng) với DQN là cơ chế/xấp xỉ hàm cho Q-learning; phụ lục luyện sâu | Nhận BC/Q/DQN từ mạch 6 | Dừng lõi tại L13-41; nếu dạy phụ lục, đi xuống vào X01 rồi tiếp tục xuống X04 |
...
- L13-28 gộp vấn đề với hình thức vì cập nhật một mẫu là câu trả lời trực tiếp cho kỳ vọng môi trường chưa biết.
- L13-29–30 gộp trực giác với triển khai: phân biệt hai chính sách rồi dùng epsilon-tham lam để tạo độ phủ.
...
| L13-15 | 3 | Ví dụ | Vòng tương tác → ba chuyển tiếp cụ thể | “Gọi tên năm thành phần và chính sách trước khi ký hiệu hóa.” |
| L13-16 | 2 | Ví dụ + kiểm tra chẩn đoán | Chuyển tiếp → năm thành phần LLO26 và chính sách tách khỏi tuple MDP | “Bây giờ đóng gói môi trường thành MDP.” |
...
| L13-20 | 2 | Triển khai + kiểm tra | Chuyển tiếp → $D^{term}$ và phần lợi tức còn lại bằng 0 | “Định nghĩa giá trị kỳ vọng.” |
...
| L13-24 | 2 | Vấn đề + trực giác | Hai kiểu mục tiêu hồi quy: lợi tức đầy đủ → giá trị ước lượng bước kế tiếp | “Viết quan hệ dưới chính sách đang xét.” |
| L13-25 | 2 | Hình thức | Đích TD → Bellman của chính sách; nối $V^\pi$ với kỳ vọng của $Q^\pi$ | “Thử một chuyển tiếp cụ thể.” |
...
| L13-28 | 2 | Vấn đề + hình thức | Bellman → cập nhật Q-learning một mẫu bằng ước lượng Q hiện tại và mặt nạ kết thúc | “Dữ liệu và chính sách đích có thể khác.” |
| L13-29 | 2 | Trực giác | $\mu$ và max-target → học ngoài chính sách, vẫn cần độ phủ | “Chính sách hành vi tạo độ phủ thế nào?” |
| L13-30 | 2 | Triển khai | Q hiện tại → epsilon-tham lam, thăm dò đều trên $\mathcal A$ | “Tính một cập nhật bảng.” |
...
| L13-34 | 2 | Trực giác | Thu thập→lưu→lấy mẫu→$Q_\theta$ hiện tại và $Q_{\bar\theta}$ đóng băng→mất mát | “Khóa tensor của từng nhánh.” |
...
| L13-36 | 2 | Hình thức + ví dụ | Chỉ số từ 0; $Q_\theta(s_1)=[.1,.4]$ → $q_{sa}=.4$; lô B | “Nhánh đích tạo Y.” |
| L13-37 | 2 | Hình thức + ví dụ | Giả sử $\max Q_{\bar\theta}(s_2)=1$ → $Y=.9$ trong ngữ cảnh không gradient | “Hai nhánh gặp nhau ở mất mát.” |
...
| L13-40 | 2 | Triển khai | Lấy mẫu, đích, đặt gradient về 0, lan truyền ngược, cập nhật và chu kỳ đồng bộ cứng $C$ | “Tự rà nguồn tín hiệu và gradient.” |
| L13-41 | 2 | Kiểm tra | Hai nguồn tín hiệu; DQN là xấp xỉ hàm cho Q-learning; BC/DAgger/RL xử lý phân phối trạng thái khác nhau | “Dừng lõi hoặc đi sang phụ lục.” |
| L13-X01 | 5 | Hình thức + kiểm tra mở rộng | Gaussian theo một mẫu $D_a$ và theo lô $B\times D_a$; $\sigma>0$; giới hạn một mode | “Tiếp tục với kiểm tra cờ kết thúc.” |
