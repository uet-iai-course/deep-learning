...
| L12-04 | 3 | VĐ: đọc hai phía mà không đọc khóa đệm; TG: hàng truy vấn/cột khóa; VD: mỗi truy vấn hợp lệ đọc các khóa hợp lệ; HT: $M^{valid}$ phát theo trục truy vấn; UD: Boolean hoặc cộng, loại vị trí đệm khỏi loss; KT: KAD vì L08 kiểm. | $N,T_q,T_k$ → mặt nạ $N\times1\times T_q\times T_k$. | Áp dụng vào vị trí che; `↓` L12-05. |
